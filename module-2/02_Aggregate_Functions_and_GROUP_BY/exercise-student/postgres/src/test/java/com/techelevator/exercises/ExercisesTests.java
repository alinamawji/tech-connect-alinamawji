...
//		jdbcTemplate.update(sql);
	...
	private void checkAnswerForProblem(String problemNumber) {
		...
		assertFalse("No query found for this exercise. Make sure you've saved your changes to the exercise file.", sqlActual.trim().length() == 0);
	...
	private String getStudentQuery(String problemNumber) {
		...
		try {
			sql = new String(Files.readAllBytes(exerciseFile.toPath()));
		}
		catch (Exception e) {
	...
	private String getSolutionQuery(String problemNumber) {
		...
		try {
			sql = new String(Files.readAllBytes(solutionFile.toPath()));
		}
		catch (Exception e) {
