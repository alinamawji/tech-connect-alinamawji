...
1. Create an `EmployeeProjects` database in Postgres. 
   - You are using the Spring Framework for this exercise. The database will be populated for you automatically each time you run the application. The SQL statements are located in the `src/main/resources` folder in a file named `data.sql`. 
   - There is no reason to run the SQL the populate the database.
   - Note that the `timesheet` table is not used in today's exercise.*
2. [Import](https://book.techelevator.com/jpmc/content/guides/intellij.html#import-a-project) the DAO exercises project into IntelliJ.
3. [Run all tests](https://book.techelevator.com/jpmc/content/guides/intellij.html#running-tests) to see the results of your tests and which ones passed or failed.
...
You should also familiarize yourself with the database schema either by looking at the database in DbVisualizer/Intellij or the `java/src/main/resouces/data.sql` script.
