...
public class JdbcDepartmentDaoTests {
    ...
    @Test
    public void updated_department_has_expected_name_when_retrieved() throws SQLException {
        Department department = getDepartment(1L);
        department.setName("Test");
        sut.updateDepartment(department);
        Department updatedDept = getDepartment(1L);
    ...
    private Department getDepartment(Long id) {
        String getOneSql = "select * from department where department_id = ?";
        JdbcTemplate t = new JdbcTemplate(dataSource);
        try {
            return t.queryForObject(getOneSql, new DepartmentRowMapper(), new Object[]{id});
